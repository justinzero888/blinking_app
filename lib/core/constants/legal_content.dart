...
Blinking 包含一项可选的 AI 助手功能。该功能默认关闭，仅在您主动配置后才会启用。

使用 AI 助手需要您提供来自受支持的第三方 AI 服务商（如 OpenAI、Anthropic、Google 或 OpenRouter）的自有 API Key。

重要说明：

• 当您向 AI 助手发送消息时，该消息（以及您选择作为上下文的日记内容）将通过您提供的 API Key 传输至第三方 AI 服务商。
• 您的数据直接从您的设备发送至 AI 服务商的服务器。Blinking 不作为中间方，不查看或存储这些数据。
• 第三方 AI 服务商可能会根据其自身的隐私政策和服务条款收集、处理和存储您发送的数据。使用该功能前，请阅读您所选 AI 服务商的隐私政策。
• 您对自己提供的 API Key 负全责，请妥善保管。
• 您可随时在设置中移除 API Key 以停用 AI 功能。
...
• 创建和管理记忆卡片
• 查看个人统计与摘要
• 使用您自有的 API Key 与第三方 AI 助手互动（可选）
...
本应用包含可选的 AI 助手功能。使用该功能需提供来自受支持的第三方 AI 服务商的有效 API Key。

使用 AI 助手功能，即表示您确认并同意：

• 您负责自行获取和管理 API Key。我们对您使用第三方 AI 服务所产生的任何费用、收费或后果不承担责任。
• 您发送给 AI 助手的内容将传输至您配置的 AI 服务商，该传输受该服务商自身的服务条款和隐私政策约束，而非本条款。
