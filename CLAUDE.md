...
### Card Builder (v1.2.0)
`CardBuilderSheet` — open via `CardBuilderSheet.show(context, initialContent: ...)`. Accepts optional `renderFn` parameter for testing. On save: renders PNG → persists `NoteCard` via `CardProvider.addCard()`. Entry points: `EntryDetailScreen` (🖼️ icon in AppBar), `AssistantScreen` (post-save icon in AppBar).
...
| **Keepsake cards — Phase 3** (8 templates, single-page, photo+text, entry badge, re-render-on-restore, CardRenderService with 4 layouts + 6 decorative motifs, CardBuilderSheet, CardPreviewScreen, 2 entry points) | ✅ Done (May 23, 2026) |
